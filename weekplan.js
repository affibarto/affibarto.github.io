...
document.addEventListener("click",function(e){var t0=e.target;if(t0&&t0.nodeType===3)t0=t0.parentNode;if(t0&&t0.closest&&t0.closest("[data-go]"))closePlan();if(t0&&((t0.id==="wpclose")||(t0.closest&&t0.closest("#wpclose")))){ensure();getWeek()._later=1;if(typeof save==="function")save();closePlan();return;}if(t0&&((t0.id==="wpsave")||(t0.closest&&t0.closest("#wpsave")))){ensure();getWeek()._done=1;if(typeof save==="function")save();afterPlanSaved();return;}if(t0&&((t0.id==="wpprimary")||(t0.closest&&t0.closest("#wpprimary")))){e.preventDefault();if(!filled())openPlan();else if(typeof openSuggest==="function")openSuggest();else openPlan();return;}if(t0&&((t0.id==="wpopen")||(t0.closest&&t0.closest("#wpopen")))){e.preventDefault();openPlan();return;}var t=e.target.closest&&e.target.closest("[data-act]");if(!t)return;var act=t.getAttribute("data-act");if(act==="wpremind"){ensure();S.planRemind=t.getAttribute("data-d");if(typeof save==="function")save();huisRemind();var row=document.getElementById("planremind");if(row)row.querySelectorAll(".chip").forEach(function(c){c.classList.toggle("on",c.getAttribute("data-d")===S.planRemind);});return;}if(act==="wpcook"){var cid=+t.getAttribute("data-id");if(!cookPool().some(function(p){return p.id===cid;}))return;slot(t.getAttribute("data-d")).cook=cid;if(typeof save==="function")save();drawPlan();e.stopPropagation();return;}if(act==="wpmode"){slot(t.getAttribute("data-d")).who[+t.getAttribute("data-id")]=t.getAttribute("data-m");if(typeof save==="function")save();drawPlan();e.stopPropagation();return;}},true);
document.addEventListener("change",function(e){var t=e.target;if(t&&t.getAttribute("data-act")==="wpguests"){var n=+t.value||0;if(n<0)n=0;if(n>12)n=12;slot(t.getAttribute("data-d")).guests=n;if(typeof save==="function")save();}});
ensure();
})();
