...
function end(){if(!cur)return;var wrap=cur,d=wrap.getAttribute("data-d");var row=wrap.querySelector(".dayrow");if(row)row.style.transition="transform .15s ease";if(dx>88){if(row)row.style.transform="translateX(100%)";window._bswiped=1;setTimeout(function(){wipeDay(d);},140);}else if(dx>12){window._bswiped=1;if(row)row.style.transform="";}else if(row)row.style.transform="";cur=null;dx=0;}
...
document.addEventListener("mousedown",function(e){if(e.target.closest&&e.target.closest("#days")){if(grab(e.clientX,e.target)){window._bdrag=1;}}},true);
