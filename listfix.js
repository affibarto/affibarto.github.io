...
function handle(e){if(!inList(e.target))return;var q=e.target.closest("[data-act=qty]");if(q){e.preventDefault();e.stopPropagation();var btn=q.closest("[data-act=qty]")||q;go(function(){if(typeof bumpExtra==="function")bumpExtra(+btn.getAttribute("data-i"),+btn.getAttribute("data-d"));});return;}var t=e.target.closest("[data-act=tog],.ck");if(t){e.preventDefault();e.stopPropagation();var k=t.getAttribute("data-k")||(t.closest(".swipewrap")&&t.closest(".swipewrap").getAttribute("data-k"));go(function(){S.checked=S.checked||{};S.checked[k]=!S.checked[k];if(typeof save==="function")save();if(typeof drawList==="function")drawList();});}}
